...
int decfile(FILE *fin, FILE *fout, aes_decrypt_ctx ctx[1], const char* ifn, const char* ofn)
{   char            dbuf[3 * BLOCK_LEN], buf[BLOCK_LEN];
    unsigned long   i, len, wlen = BLOCK_LEN;
    ...
    {   char    *b1 = dbuf, *b2 = b1 + BLOCK_LEN, *b3 = b2 + BLOCK_LEN, *bt;
        ...
        for( ; ; )  // while some ciphertext remains, prepare to decrypt block b2
        {
            ...
            if(fwrite(buf, 1, wlen, fout) != wlen)
                return WRITE_ERROR;
